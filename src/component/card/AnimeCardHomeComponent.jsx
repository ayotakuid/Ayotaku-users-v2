...
import SkeletonCardAnimeComponent from "../utils/SkeletonCardAnimeComponent";

function AnimeCardHomeComponent({ animes }) {
  const [isAnimes, setAnimes] = useState([]);
  const [isLoading, setLoading] = useState(true); // default-nya true dulu
  ...
  useEffect(() => {
    ...
    // simulasi loading (misalnya ambil data dari props yang delay)
    setLoading(true);
    const timeout = setTimeout(() => {
      setAnimes(animes);
      setLoading(false); // matikan loading setelah selesai set data
    }, 1000);
    ...
    <>
      {isLoading ? (
        Array.from({ length: 18 }).map((_, index) => (
          <SkeletonCardAnimeComponent key={index} />
        ))
      ) : (
        isAnimes?.map((items) => (
          <div
          ...
          >
            <div className="relative w-full">
              ...
              <span className="flex items-center absolute top-2 right-2 w-fit bg-black bg-opacity-50 text-ayotaku-text-default px-2 py-1 rounded-md text-ayotaku-text-sm gap-1 group-hover:opacity-70 duration-500">
                ...
                <span className="text-ayotaku-text-default">
                  {checkingRating(items.data.rating)}
                </span>
              </span>
              ...
              <span className="flex items-center absolute bottom-2 left-2 w-fit bg-black bg-opacity-50 text-ayotaku-text-default px-2 py-1 rounded-md text-ayotaku-text-sm gap-1 group-hover:opacity-70 duration-500">
                <span className="text-ayotaku-text-default">
                  {checkingAiring(items.data.status.statusAiring)}
                </span>
              </span>
            </div>
